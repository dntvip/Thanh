...
(function() {
  window.addEventListener('load', function() {
    const moaElement = document.getElementById('moa');
    
    // The main canvas is globally available as `canvas.canvas`
    if (canvas && canvas.canvas && moaElement) {
      canvas.canvas.addEventListener('click', (event) => {
        // If the animation is running, don't do anything
        if (moaElement.classList.contains('show')) {
          return;
        }

        // Position 'moa' text at the click coordinates
        moaElement.style.left = `${event.clientX}px`;
        moaElement.style.top = `${event.clientY}px`;
        moaElement.style.transform = 'translate(-50%, -150%)'; // Adjust position relative to cursor

        moaElement.classList.add('show');

        setTimeout(() => {
          moaElement.classList.remove('show');
